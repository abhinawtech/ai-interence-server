...
impl TinyLlamaModel {
    ...
    async fn load_model_components(device: &Device) -> Result<(Llama, Tokenizer, Config)> {
        ...
        let (model, tokenizer, config) = tokio::task::spawn_blocking(move || {
            Self::load_model_sync(device_clone)
        }).await??;
    ...
    fn load_model_sync(device: Device) -> Result<(Llama, Tokenizer, Config)> {
        ...
        tracing::info!("Parsed config: vocab_size={}, hidden_size={}, num_layers={}", 
    ...
    fn load_weights<'a>(weight_files: &'a [std::path::PathBuf], device: &'a Device) -> Result<VarBuilder<'a>> {
        // YOUR EXISTING WEIGHT LOADING LOGIC
        if weight_files[0].extension().and_then(|s| s.to_str()) == Some("safetensors") {
            tracing::info!("Loading safetensors weights...");
            unsafe {
                Ok(VarBuilder::from_mmaped_safetensors(weight_files, DType::F16, device)?)
            }
        } else {
    ...
    pub async fn generate(&mut self, prompt: &str, max_tokens: usize) -> Result<String> {
        tracing::debug!("Generating text for prompt: '{}' (max_tokens: {})", prompt, max_tokens);
        ...
        tracing::debug!("Input tokens: {:?} (length: {})", input_ids, input_ids.len());
        ...
        tracing::debug!("Created input tensor with shape: {:?}", input_tensor.shape());
        ...
        tracing::debug!("Generated {} new tokens: {:?}", generated_tokens.len(), generated_tokens);
    ...
    fn is_eos_token(&self, token: u32) -> bool {
        ...
        if let Some(candle_transformers::models::llama::LlamaEosToks::Single(eos_id)) = &self.config.eos_token_id {
            token == *eos_id
        } else {
